...
const Chartt = () => {
  ...
  const handleFilterButtonClick = () => {
    const newFilteredData = positiveVoltages.filter((voltage, index) => {
      const timestamp = positiveTimeStamps[index];
      return timestamp >= start && timestamp <= end && voltage > 10;
    });
  ...
  useEffect(() => {
    const chartData = {
      ...
      datasets: [
        {
          label: "Positive Voltage",
          data: filteredData.length ? filteredData : positiveVoltages,
          borderColor: "rgba(54, 162, 235, 1)",
          backgroundColor: "rgba(54, 162, 235, 0.2)",
          borderWidth: 1
        },
